...
the last file_list.txt with the current file_list.txt on the FTP
server.
...
parser.add_option("-c", "--check", dest="check", action="store_true", \
    help="do not use file_list.txt to find the right files, but check file existence on disk")
parser.add_option("", "--justCheck", dest="justCheck", action="store_true", \
    help="just check if all files in the current file_list.txt are really on disk")
...
def parseFileList(path):
    ...
    for line in ifh:
        fields = line.strip('\n').split("\t")
        if len(fields)!=3:
            fname, cit, pmcId, pdfName = fields # PMC changed their format for a few weeks
        else:
...
def updatePmc(outDir, checkOnDisk):
    ...
    if checkOnDisk is True, always check files on disk instead of using the file_list.txt comparison
    """

    # download from server to .current.txt
    logging.debug("Inferring missing PMC-IDs by comparing file_list.txt")
    ...
    listName = "file_list.txt"
    downloadOk = util.ftpDownload(ftpConn, listName, newListPath)
    if not downloadOk:
        raise Exception("could not download file_list.txt")
...
def checkPmc(outDir):
    ...
    for pmcId, fileName in ourPmc.iteritems():
        ...
        if not isfile(locFname):
            ...
            missCount +=1
    logging.error("%d file missing" % missCount)
...
def main(args, options):
    ...
    if not isdir(outDir):
        logging.info("%s does not exist" % outDir)
        sys.exit(1)
